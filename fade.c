/* ==============================================================
 *  ORIGINAL
 *	http://forum.arduino.cc/index.php?topic=102040.0
*/ 
...
// time as we only have one loop index.
{
	...
	for (i = 0; i < MAX_RGB_VALUE-MIN_RGB_VALUE; i++, v.x += dx, v.y += dy, v.z += dz)
	{
		// set the colour in the LED
		printf("[%d,%d,%d],", v.x, v.y, v.z);
		
		usleep(TRANSITION_DELAY);  // wait fot the transition usleep
	}

	usleep(WAIT_DELAY);          // give it an extra rest at the end of the traverse
}

void loop()
{
	int    v1, v2=0, i;    // the new vertex and the previous one
