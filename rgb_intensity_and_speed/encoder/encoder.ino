...
int button_manual;
int auto_direction;
int active_encoder_function;
int encoder_has_action;
...
void test_auto_manual_button() {
  ...
  if(button_manual == LOW && button_auto == LOW) {
   test();
  }

  if(button_manual == HIGH) {
   write_rgb(RED);
   delay(300);
   write_rgb(BLACK);
  }
...
void setup() {
  ...
  active_encoder_function = NONE;
  encoder_has_action = 0;
  transition_delay = 20;
  timer = 0;
...
void loop() {
  ...
  if(button_auto == HIGH) {
    unsigned long time = millis() - timer;
    
    if(time > TIMER_TIME_OUT || active_encoder_function == NONE) {
      active_encoder_function = NONE;
      encoder_has_action = 0;
    }
    
    if(dir != 0 && encoder_has_action == 0) {
      encoder_has_action = 1;
      Serial.println("has action");
      
      if(dir == 1) {
        active_encoder_function = CHANGE_INTENSITY;
      } else {
        active_encoder_function = CHANGE_TRANSITION_SPEED;
      }
    }
     
    if(encoder_has_action == 1 && dir != 0) {
      if(active_encoder_function == CHANGE_INTENSITY) {
         if(current_color.x != 0 && current_color.x + dir != 0 &&
              current_color.y != 0 && current_color.y + dir != 0 &&
              current_color.z != 0 && current_color.z + dir != 0 &&
              current_color.x + dir < 256 && 
              current_color.x + dir < 256 &&
              current_color.x + dir < 256) {
           current_color.x += dir;
           current_color.y += dir;
           current_color.z += dir;
           Serial.println("Changed intensity");
         }
      }
      
      if(active_encoder_function == CHANGE_TRANSITION_SPEED && dir != 0) {
         if(!(transition_delay + dir > MAX_TRANSITION_DELAY || 
              transition_delay + dir < MIN_TRANSITION_DELAY)) {
           transition_delay += dir;
           Serial.print("Changed delay: ");
           Serial.println(transition_delay);
         }   
      }
    }
    ...
    if(target == AUTO_UPPER_BOUND) {
      auto_direction = -1;
    }
    
    next_color(auto_direction);
    write_rgb(current_color);
    
    delay(transition_delay);
  }
  
  if(button_manual == HIGH && dir != 0) {
     active_encoder_function = CHANGE_COLOR; 
