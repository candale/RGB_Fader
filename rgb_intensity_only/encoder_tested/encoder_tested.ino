...
/* takes a value and retruns -1 if below 0, 1 if above 255, and 0 if in between */
#define IN_BOUND(a) (a < 0 ? -1 : a > 255 ? 1 : 0)
...
void write_rgb(struct color c) {
  int x, y, z;
  x = c.x + color_intensity; 
  y = c.y + color_intensity;
  z = c.z + color_intensity;

  if(IN_BOUND(x) == -1) {
    x = 0;
  } else if(IN_BOUND(x) == 1) {
    x = 255;
  }

  if(IN_BOUND(y) == -1) {
    y = 0;
  } else if(IN_BOUND(y) == 1) {
    y = 255;
  }

  if(IN_BOUND(z) == -1) {
    z = 0;
  } else if(IN_BOUND(z) == 1) {
    z = 255;
  }
...
void loop() {
  ...
  if(button_manual == HIGH && dir != 0) {
    next_color(ZSGN(encoder_changed));
  ...
  if(button_auto == HIGH) {
    ...
    if(millis() - timer > TRANSITION_DELAY) {
